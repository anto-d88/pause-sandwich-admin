...
export default function KitchenPage({ activePage, setActivePage }) {
  ...
  async function loadKitchen() {
    ...
    if (!teamOrdersResult.error)
      setTeamOrders(teamOrdersResult.data || []);
    if (!orderItemsResult.error)
      setOrderItems(orderItemsResult.data || []);
    if (!teamItemsResult.error)
      setTeamOrderItems(teamItemsResult.data || []);

    if (ordersResult.error)
      console.error("orders", ordersResult.error);

    if (teamOrdersResult.error)
      console.error("team_orders", teamOrdersResult.error);

    if (orderItemsResult.error)
      console.error("order_items", orderItemsResult.error);

    if (teamItemsResult.error)
      console.error("team_order_items", teamItemsResult.error);
  ...
  function getItems(order) {
    if (order.order_type === "team") {
      return teamOrderItems.filter(
        (item) =>
          Number(item.team_order_id) === Number(order.id)
      );
    }
  ...
  function getItemDetails(item) {
    ...
    if (item.participant_name)
      details.push(`👤 ${item.participant_name}`);

    if (item.item_type)
      details.push(`Type : ${item.item_type}`);

    if (item.boisson_name)
      details.push(`Boisson : ${item.boisson_name}`);

    if (item.dessert_name)
      details.push(`Dessert : ${item.dessert_name}`);

    if (item.crudites)
      details.push(`Crudités : ${item.crudites}`);

    if (item.sauce)
      details.push(`Sauce : ${item.sauce}`);

    if (item.comment)
      details.push(`Note : ${item.comment}`);
      ...
      <div className="mb-8 flex flex-col gap-4 xl:flex-row xl:items-center xl:justify-between">
        <div>
          ...
          <p className="mt-2 text-slate-400">
            Écran de production :
            nouvelles commandes,
            préparation et commandes terminées.
          </p>
        </div>
